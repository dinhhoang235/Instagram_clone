...
export default function ProfileClient({ username, defaultTab = "posts" }: ProfileClientProps) {
    ...
    // sync activeTab -> URL
    useEffect(() => {
        const path = activeTab === 'posts' ? `/${username}` : `/${username}/${activeTab}`
        // update URL without triggering a full navigation to keep tab switching smooth
        if (typeof window !== 'undefined') {
            if (window.location.pathname !== path) {
                window.history.replaceState({}, '', path)
            }
        } else {
            // fallback to router.replace on server-side/unknown environments
            router.replace(path)
        }
