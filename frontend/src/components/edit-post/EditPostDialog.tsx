  ...
  return (
    ...
    >
      <DialogPortal>
        ...
        <DialogPrimitive.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-[1000px] h-[700px] max-h-[90vh] bg-black text-white rounded-2xl overflow-hidden shadow-2xl">
          ...
          {/* Header */}
          <div className="relative bg-black text-white px-4 h-11 border-b border-zinc-800 flex items-center justify-between">
            <button
              onClick={() => { if (!isSaving) onOpenChange(false) }}
              className="text-white hover:opacity-70 transition-opacity font-normal"
              aria-label="Close"
            >
              Cancel
            </button>
          ...
          <div className="flex h-[calc(100%-44px)]">
            ...
            >
              {isLoading ? (
                ...
                <div className="flex items-center justify-center flex-col text-zinc-500">
            ...
            <div className="w-[340px] bg-[#262626] flex flex-col">
              {/* User Info */}
              <div className="p-4 flex items-center gap-3 border-b border-zinc-700">
                ...
                <span className="text-sm font-semibold text-white">{postUser?.username}</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                <div className="border-b border-zinc-700">
                  <div className="relative p-4">
                    <Textarea 
                      ...
                      className="w-full min-h-[180px] bg-transparent border-0 text-white placeholder:text-zinc-500 resize-none p-0 ring-0 focus:ring-0 focus-visible:ring-0 ring-offset-0 focus:ring-offset-0 focus-visible:ring-offset-0 overflow-y-auto"
                    />
                    
                    <div className="flex items-center justify-between mt-2 border-t border-zinc-700 pt-3">
                      <button
                        type="button"
                      ...
                      <span className="text-xs text-zinc-500">{caption.length}/2,200</span>
                    </div>
                ...
                <div className="border-b border-zinc-700">
                  ...
                  >
                    <span className={`text-base ${showAccessibility ? 'font-semibold' : 'font-normal'} text-white`}>Accessibility</span>
                    <ChevronDown 
                      className={`w-5 h-5 text-zinc-500 transition-transform ${showAccessibility ? 'rotate-180' : ''}`} 
                    />
                  </div>
                  ...
                  {showAccessibility && (
                    <div className="px-4 pb-4 space-y-3">
                      ...
                      {images.map((src, idx) => (
                        <div key={idx} className="flex items-center gap-3 bg-zinc-800/50 rounded-lg p-2">
                      ...
                      {images.length === 0 && (
                        <div className="text-xs text-zinc-500">No images to add alt text for.</div>
                      )}
                    </div>
                  )}
                </div>
